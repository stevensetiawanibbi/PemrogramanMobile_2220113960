...
void main() {
  runApp(const ForeCoffeeLogin());
}

class ForeCoffeeLogin extends StatelessWidget {
  const ForeCoffeeLogin({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Fore Coffee Login',
      debugShowCheckedModeBanner: false,
      home: const LoginPage(),
      theme: ThemeData(
        // sesuaikan jika mau
        primaryColor: const Color(0xFF2F4F2F),
        fontFamily: 'Roboto',
      ),
    );
  }
              ...
              child: ElevatedButton(
                onPressed: _isButtonEnabled
                    ? () {
                        ...
                        // contoh: navigasi atau request OTP
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('Lanjutkan dengan ${_phoneController.text}')),
                        );
                      }
